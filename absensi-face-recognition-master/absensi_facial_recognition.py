...
window.title("Absensi Mahasiswa Face Recognition")
...
def set_kelas():
    ...
    frame3 = ttk.Frame(tab_parent)
    frame4 = ttk.Frame(tab_parent)
    tab_parent.add(frame1, text="Tambah Kelas")
    tab_parent.add(frame2, text="Hapus Kelas")
    tab_parent.add(frame3, text="Tambah Matkul")
    tab_parent.add(frame4, text="Tambah Dosen")
    ...
    framez = Frame(frame3)
    framez.pack(side=TOP,anchor=CENTER, pady=30)
    labelmatkul = Label(framez, text="Matkul (gunakan koma jika lebih dari satu) :", font=('helvetica', 10))
    labelmatkul.pack(side=TOP, anchor=CENTER)
    entrymatkul = Entry(framez,width=40, font=('helvetica', 10))
    entrymatkul.pack(side=TOP, anchor=CENTER,pady=5)
    framey = Frame(frame4)
    framey.pack(side=TOP,anchor=CENTER, pady=30)
    labeldosen = Label(framey, text="Dosen (gunakan koma jika lebih dari satu) :", font=('helvetica', 10))
    labeldosen.pack(side=TOP, anchor=CENTER)
    entrydosen = Entry(framey,width=40, font=('helvetica', 10))
    entrydosen.pack(side=TOP, anchor=CENTER,pady=5)
    ...
    def input_nama_kelas():
        ...
        if nama_kls == '':
            ...
            with open('Mahasiswa/daftarkelas.csv', 'a+', newline='') as csvfile:
                ...
                kelaswin.focus_set() 
    def input_nama_matkul():
        nama_matkul = (entrymatkul.get())
        if nama_matkul == '':
            mb.showerror('Tidak ada Input', 'Input Matkul belum dimasukkan')
            kelaswin.focus_set()
        else:
            result = [x.strip() for x in nama_matkul.split(',')]
            #res = [[result[0], x] for x in result[1:]]
            if not os.path.isdir('Mahasiswa/'):
                os.mkdir('Mahasiswa/')
            with open('Mahasiswa/daftarmatkul.csv', 'a+', newline='') as csvfile:
                writer = csv.writer(csvfile, delimiter='\n') 
                #writer.writerow(','.join(Tup1))
                writer.writerow(result)  
                csvfile.close()
                readdelete()
                read_kelas()
                mb.showinfo("Input Matkul", "Matkul berhasil dimasukkan") 
                kelaswin.focus_set() 
        
    def input_nama_dosen():
        nama_dosen = (entrydosen.get())
        if nama_dosen == '':
            mb.showerror('Tidak ada Input', 'Input Dosen belum dimasukkan')
            kelaswin.focus_set()
        else:
            result = [x.strip() for x in nama_dosen.split(',')]
            #res = [[result[0], x] for x in result[1:]]
            if not os.path.isdir('Mahasiswa/'):
                os.mkdir('Mahasiswa/')
            with open('Mahasiswa/daftardosen.csv', 'a+', newline='') as csvfile:
                writer = csv.writer(csvfile, delimiter='\n') 
                #writer.writerow(','.join(Tup1))
                writer.writerow(result)  
                csvfile.close()
                readdelete()
                read_kelas()
                mb.showinfo("Input Dosen", "Nama Dosen berhasil dimasukkan") 
                kelaswin.focus_set() 
    ...
    buttonmatkul = Button(framez, text="Tambah", font=('helvetica', 9), width = 10, command=input_nama_matkul)
    buttondosen = Button(framey, text="Tambah", font=('helvetica', 9), width = 10, command=input_nama_dosen)
    ...
    buttonmatkul.pack(side= TOP, padx=10)
    buttondosen.pack(side= TOP, padx=10)
...
def viewabs():
        ...
        try:
            with open(pathabs) as f:
                ...
                for row in reader:
                    ...
                    Matkul = row['Matkul']
                    Dosen = row['Dosen']
                    tree.insert("", 0, values=(NPM, Nama, Tanggal, Waktu,Matkul,Dosen))
        except(FileNotFoundError):
    ...
    scrollbarx = Scrollbar(TableMargin, orient=HORIZONTAL)
    scrollbary = Scrollbar(TableMargin, orient=VERTICAL)
    tree = ttk.Treeview(TableMargin, columns=("NPM", "Nama", "Tanggal", "Waktu","Matkul","Dosen"), height=200, selectmode="extended", yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
    scrollbary.config(command=tree.yview)
    scrollbary.pack(side=RIGHT, fill=Y)
    scrollbarx.config(command=tree.xview)
    scrollbarx.pack(side=BOTTOM, fill=X)
    ...
    tree.heading('Matkul', text="Matkul", anchor=W)
    tree.heading('Dosen', text="Dosen", anchor=W)
    tree.column('#0', stretch=NO, minwidth=0, width=0)
...
def tentang():
    mb.showinfo('Absensi Facial Recognition', 'Pengembang aplikasi Reffian Oktarino')
...
dropdown_file.add_command(label='Kelas, Matkul, Dosen...',command=set_kelas)
dropdown_file.add_separator()
dropdown_file.add_command(label=' Set Port Kamera USB...',command = set_port)
dropdown_file.add_separator()
...
def train_intro():
    ...
    try:
        ...
        kombo_kelas['values']=('')
 #1
    label_matkul = Label(intro_train, text='Cari Matkul: ',font=('helvetica', 13))
    label_matkul.pack(side=TOP, pady=5)
    kombo_matkul = ttk.Combobox(intro_train, width =20,state="readonly",font=('helvetica', 13))
    kombo_matkul.pack(side=TOP, anchor = CENTER)

    kombo_matkul['values']=('')
    try:
        with open('Mahasiswa/daftarmatkul.csv', newline='') as f:
            data = list(csv.reader(f))
            new_data = [a for i, a in enumerate(data) if a not in data[:i]]
            with open('Mahasiswa/daftarmatkul.csv', 'w', newline='') as t:
                write = csv.writer(t)
                write.writerows(new_data)

        with open('Mahasiswa/daftarmatkul.csv', newline='') as csvfile:
            spamreader = csv.reader(csvfile , delimiter='"') 
            sortedlist = sorted(spamreader, key=operator.itemgetter(0))
            Tup1 = ()
            Lst1 = ()
        ...
        kombo_matkul['values']=(Tup1)
        
    except(FileNotFoundError):
        kombo_matkul['values']=('')
    #2
    label_dosen = Label(intro_train, text='Cari Dosen: ',font=('helvetica', 13))
    label_dosen.pack(side=TOP, pady=5)
    kombo_dosen = ttk.Combobox(intro_train, width =20,state="readonly",font=('helvetica', 13))
    kombo_dosen.pack(side=TOP, anchor = CENTER)

    kombo_dosen['values']=('')
    try:
        with open('Mahasiswa/daftardosen.csv', newline='') as f:
            data = list(csv.reader(f))
            new_data = [a for i, a in enumerate(data) if a not in data[:i]]
            with open('Mahasiswa/daftardosen.csv', 'w', newline='') as t:
                write = csv.writer(t)
                write.writerows(new_data)

        with open('Mahasiswa/daftardosen.csv', newline='') as csvfile:
            spamreader = csv.reader(csvfile , delimiter='"') 
            sortedlist = sorted(spamreader, key=operator.itemgetter(0))
            Tup1 = ()
            Lst1 = ()
...
def kenaliwajah(kelas, penyimpanan):
    try:
        ...
        cam = cv2.VideoCapture(portnum.get())
        font = cv2.FONT_HERSHEY_SIMPLEX        
        nama_kolom =  ['NPM','Nama','Tanggal','Waktu','Matkul','Dosen']
        ...
        while True:
            ...
            for(x,y,w,h) in mukamuka:
                ...
                if(conf < 40):
                    ...
                    absensi.loc[len(absensi)] = [nomormhs,aa,date,timeStamp,matkul,dosen]
...
button_train.place(x=370, y=180)
...
button_switch.place(x=200, y=180)
